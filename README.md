# vscode-fuzzy-finder README

This is the README for your extension "vscode-fuzzy-finder". After writing up
a brief description, we recommend including the following sections.

## Idea

Fuzzy find functionality in modal window

VScode doesn't actually allow natively modal windows, but we could use
something similar [vscode-color](https://github.com/anseki/vscode-color)
extension does; it opens its own process/window and uses IPC to communicate
with VSCode. --- UPDATE: Actually a
[webview](https://code.visualstudio.com/api/extension-guides/webview) could be
a solution also.

The fuzzy finding implementation could follow same method that
[vscode-finditfaster](https://github.com/tomrijndorp/vscode-finditfaster)
uses; just use already existing tools like `fzf` and `rgrep`.
